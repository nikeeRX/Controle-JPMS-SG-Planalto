...
async def login(request: Request):
    ...
    with engine.connect() as conn:
        ...
        if user:
            if user[2] == 'BLOQUEADO': return HTMLResponse("<script>alert('Acesso Bloqueado ou Pendente!'); window.location.href='/';</script>")
            request.session["user"], request.session["role"] = user[0], user[1]
...
async def menu_secretaria(request: Request):
    ...
    with engine.connect() as conn:
        ...
        for r in itens_db:
            btn = f"<form action='/secretaria/excluir' method='post' style='margin:0;' onsubmit='return confirm(\"Excluir item?\");'><input type='hidden' name='id' value='{r[0]}'><button class='btn-acao btn-red' style='padding:5px; margin:0;'>🗑️</button></form>"
            linhas_itens += f"<tr><td><b style='color:#FFF;'>{r[2]}</b><br><small style='color:#888;'>Cód: {r[1]}</small></td><td style='color:{COR_AMARELO};'>R$ {float(r[3] or 0):.2f}</td><td style='text-align:center;'>{int(r[4] or 0)} un</td><td>{btn}</td></tr>"
...
async def secretaria_loja(request: Request):
    ...
    with engine.connect() as conn:
        itens_db = conn.execute(text("SELECT id, codigo, descricao, preco, estoque FROM secretaria_itens ORDER BY descricao")).fetchall()
        for r in itens_db:
            estoque = int(r[4] or 0)
            btn = f"<form action='/secretaria/solicitar' method='post' style='margin:0;'><input type='hidden' name='id' value='{r[0]}'><button class='btn-acao' style='padding:5px; margin:0; font-size:12px;' {'disabled' if estoque <= 0 else ''}>{'SOLICITAR' if estoque > 0 else 'ESGOTADO'}</button></form>"
            linhas_itens += f"<tr><td><b style='color:#FFF;'>{r[2]}</b></td><td style='color:{COR_AMARELO};'>R$ {float(r[3] or 0):.2f}</td><td>{btn}</td></tr>"
...
async def sec_solicitar(request: Request, id: int = Form(...)):
    ...
    with engine.begin() as conn:
        item = conn.execute(text("SELECT descricao, preco, estoque FROM secretaria_itens WHERE id = :id"), {"id": id}).fetchone()
        if item and item[2] > 0:
            conn.execute(text("INSERT INTO pedidos_secretaria (username, item_nome, quantidade, valor_total, status) VALUES (:u, :n, 1, :v, 'PENDENTE')"), {"u": user, "n": item[0], "v": item[1]})
...
async def menu_tesouraria(request: Request):
    ...
    for c in minhas_caixinhas:
        cls_status = "status-pendente" if c[3] == 'PENDENTE' else "status-analise" if c[3] == 'EM ANÁLISE' else "status-pago"
        btn = f"<button class='btn-acao' style='padding:5px; font-size:12px; margin:0;' onclick='abrirModalPgto(\"caixinha\", {c[0]}, {float(c[2] or 0)})'>PAGAR</button>" if c[3] == 'PENDENTE' else ""
        linhas_caixa += f"<tr><td><b style='color:#FFF;'>{c[1]}</b></td><td style='color:{COR_AMARELO};'>R$ {float(c[2] or 0):.2f}</td><td class='{cls_status}'>{c[3]}</td><td>{btn}</td></tr>"

    linhas_pend = ""
    for p in minhas_pendencias:
        cls_status = "status-pendente" if p[3] == 'PENDENTE' else "status-analise" if p[3] == 'EM ANÁLISE' else "status-pago"
        data_formatada = p[4].strftime('%d/%m') if hasattr(p[4], 'strftime') else str(p[4])
        btn = f"<button class='btn-acao' style='padding:5px; font-size:12px; margin:0;' onclick='abrirModalPgto(\"pedido\", {p[0]}, {float(p[2] or 0)})'>PAGAR</button>" if p[3] == 'PENDENTE' else ""
        linhas_pend += f"<tr><td><b style='color:#FFF;'>{p[1]}</b><br><small style='color:#888;'>{data_formatada}</small></td><td style='color:{COR_AMARELO};'>R$ {float(p[2] or 0):.2f}</td><td class='{cls_status}'>{p[3]}</td><td>{btn}</td></tr>"
...
async def tesouraria_aprovar(request: Request):
    ...
    linhas = ""
    with engine.connect() as conn:
        cx = conn.execute(text("SELECT id, username, mes_ano as ref, valor, comprovante, 'caixinha' as tipo FROM caixinha WHERE status='EM ANÁLISE'")).fetchall()
        pd = conn.execute(text("SELECT id, username, item_nome as ref, valor_total as valor, comprovante, 'pedido' as tipo FROM pedidos_secretaria WHERE status='EM ANÁLISE'")).fetchall()
        for r in cx + pd:
            btn_ap = f"<form action='/tesouraria/processar' method='post' style='margin:0;'><input type='hidden' name='t' value='{r[5]}'><input type='hidden' name='i' value='{r[0]}'><input type='hidden' name='st' value='PAGO'><button class='btn-acao btn-green' style='padding:5px; margin:0;'>✔️ APROVAR</button></form>"
            btn_re = f"<form action='/tesouraria/processar' method='post' style='margin:0;'><input type='hidden' name='t' value='{r[5]}'><input type='hidden' name='i' value='{r[0]}'><input type='hidden' name='st' value='PENDENTE'><button class='btn-acao btn-red' style='padding:5px; margin:0;'>❌ RECUSAR</button></form>"
            linhas += f"<tr><td><b style='color:#FFF; text-transform:uppercase;'>{r[1]}</b><br><small style='color:#888;'>{r[5].upper()} - {r[2]}</small></td><td style='color:{COR_AMARELO};'>R$ {float(r[3] or 0):.2f}</td><td><a href='/comprovantes/{r[4]}' target='_blank' style='color:#0ea5e9; text-decoration:none; font-weight:bold;'>Ver Anexo 👁️</a></td><td><div style='display:flex;gap:5px;'>{btn_ap}{btn_re}</div></td></tr>"
...
async def relatorio_geral(request: Request):
    ...
    with engine.connect() as conn:
        f_bar = conn.execute(text("SELECT SUM(total_conta) FROM comandas WHERE status = 'FECHADA'")).fetchone()
        f_bar_val = float(f_bar[0]) if f_bar and f_bar[0] is not None else 0.0
        
        f_cx = conn.execute(text("SELECT SUM(valor) FROM caixinha WHERE status = 'PAGO'")).fetchone()
        f_cx_val = float(f_cx[0]) if f_cx and f_cx[0] is not None else 0.0
        
        f_sec = conn.execute(text("SELECT SUM(valor_total) FROM pedidos_secretaria WHERE status = 'PAGO'")).fetchone()
        f_sec_val = float(f_sec[0]) if f_sec and f_sec[0] is not None else 0.0
...
async def pdv_painel(request: Request):
    ...
    with engine.connect() as conn:
        ...
        for c in comandas_abertas: linhas_comandas += f"<div class='card-comanda-item' data-nome='{c[0]}' style='background:{COR_INPUT}; border:1px solid {COR_BORDA}; border-radius:8px; padding:15px; display:flex; justify-content:space-between; align-items:center;'><div><span style='font-size:18px; font-weight:bold; color:{COR_AMARELO};'>📋 {c[0].upper()}</span><br><small style='color:#888;'>Consumo Parcial</small></div><div style='text-align:right;'><span style='font-size:20px; font-weight:bold; color:#FFF;'>R$ {float(c[1] or 0):.2f}</span><br><a href='/pdv/comanda/{urllib.parse.quote(c[0])}' class='btn-acao' style='padding:5px 12px; margin:5px 0 0 0; font-size:12px; display:inline-block; width:auto;'>Lançar / Fechar</a></div></div>"
...
async def tela_comanda_detalhe(numero_comanda: str, request: Request):
    ...
    with engine.connect() as conn:
        ...
        html_itens = "".join([f"<div style='display:flex; justify-content:space-between; padding:10px; border-bottom:1px dashed {COR_BORDA}; color:#FFF; align-items:center;'><span>{it[1]}</span><span>R$ {float(it[2] or 0):.2f} <form action='/pdv/remover_item' method='post' style='display:inline; margin-left:15px;'><input type='hidden' name='item_id' value='{it[0]}'><input type='hidden' name='num_comanda' value='{numero_comanda}'><button style='background:none; border:none; color:{COR_VERMELHO}; cursor:pointer; font-size:18px;' title='Estornar Item'>☒</button></form></span></div>" for it in itens_lançados])
        
        produtos_db = conn.execute(text("SELECT id, nome, preco, estoque FROM produtos ORDER BY nome")).fetchall()
        html_produtos = ""
        for p in produtos_db:
            if p[3] > 0: html_produtos += f"<div class='card-produto' onclick='adicionarItem({p[0]})'><span style='font-weight:bold; color:{COR_AMARELO}; display:block; font-size:15px;'>{p[1].upper()}</span><span style='color:#FFF; font-weight:bold; font-size:14px; display:block; margin-top:5px;'>R$ {float(p[2] or 0):.2f}</span><span class='badge-estoque'>Estoque: {int(p[3])} un</span></div>"
            else: html_produtos += f"<div class='card-produto' style='border-color:{COR_VERMELHO}; opacity:0.6; cursor:not-allowed;'><span style='font-weight:bold; color:{COR_VERMELHO}; display:block; font-size:15px; text-decoration: line-through;'>{p[1].upper()}</span><span style='color:#FFF; font-weight:bold; font-size:14px; display:block; margin-top:5px;'>R$ {float(p[2] or 0):.2f}</span><span class='badge-estoque' style='color:{COR_VERMELHO}; font-weight:bold;'>ESGOTADO</span></div>"
...
async def pdv_adicionar_item(comanda_num: str = Form(...), produto_id: int = Form(...)):
    with engine.begin() as conn:
        ...
        if prod and prod[2] > 0:
            conn.execute(text("INSERT INTO vendas_itens (comanda_num, item_nome, valor, status) VALUES (:c, :n, :v, 'ABERTA')"), {"c": comanda_num, "n": prod[0], "v": prod[1]})
            conn.execute(text("UPDATE comandas SET total_conta = total_conta + :v WHERE numero_comanda = :c AND status = 'ABERTA'"), {"v": prod[1], "c": comanda_num})
...
async def pdv_remover_item(item_id: int = Form(...), num_comanda: str = Form(...)):
    with engine.begin() as conn:
        ...
        if item:
            conn.execute(text("UPDATE comandas SET total_conta = GREATEST(total_conta - :v, 0.00) WHERE numero_comanda = :c AND status = 'ABERTA'"), {"v": item[1], "c": num_comanda})
            conn.execute(text("UPDATE vendas_itens SET status = 'ESTORNADO' WHERE id = :id"), {"id": item_id})
            conn.execute(text("UPDATE produtos SET estoque = estoque + 1 WHERE nome = :n"), {"n": item[0]})
...
async def finalizar_comanda(request: Request, comanda_num: str = Form(...), pagamento: str = Form(...)):
    ...
    with engine.begin() as conn:
        ...
        for idx, item in enumerate(itens): txt += f"1x {item[0][:20]:<20} R$ {float(item[1] or 0):.2f}\n"
        txt += f"--------------------------------\nTOTAL: R$ {float(comanda[0] or 0):.2f}\nPAGTO: {nome_pagamento}\n Obrigado pela parceria! 🦅\n--------------------------------\n"
...
async def tela_estoque(request: Request):
    ...
    with engine.connect() as conn:
        ...
        for r in prods_db:
            n_seguro = r[1].replace('"', '').replace("'", "")
            acoes = f"<div style='display:flex; gap:5px;'><button type='button' class='btn-acao' style='padding:8px; margin:0; background:#10b981; color:#000;' onclick='abrirModalEntrada({r[0]}, \"{n_seguro}\")' title='Compra c/ NF'>➕</button><button type='button' class='btn-acao btn-dark' style='padding:8px; margin:0;' onclick='abrirModal({r[0]}, \"{n_seguro}\", \"{r[2]}\", {float(r[3] or 0)})' title='Editar'>✏️</button><form action='/excluir_produto' method='post' style='margin:0;' onsubmit='return confirm(\"Excluir item?\");'><input type='hidden' name='id' value='{r[0]}'><button class='btn-acao btn-red' style='padding:8px; margin:0;'>🗑️</button></form></div>"
            linhas += f"<tr><td style='color:#FFF; font-weight:bold;'>{r[1].upper()}</td><td style='color:{COR_AMARELO};'>R$ {float(r[3] or 0):.2f}</td><td style='color:#FFF; text-align:center;'>{int(r[4] or 0)} un</td><td>{acoes}</td></tr>"
    ...
    try:
        with engine.begin() as conn: 
            ...
            if prod:
                ...
                conn.execute(text("INSERT INTO historico_estoque (produto_nome, qtd_adicionada, valor_custo, data_entrada, nota_fiscal) VALUES (:n, :q, :c, CURRENT_DATE, :nf)"), {"n": prod[0], "q": qtd, "c": custo, "nf": filename})
...
async def historico_compras(request: Request):
    ...
    linhas = ""
    with engine.connect() as conn:
        hist = conn.execute(text("SELECT produto_nome, qtd_adicionada, valor_custo, data_entrada, nota_fiscal FROM historico_estoque ORDER BY id DESC LIMIT 100")).fetchall()
        for r in hist:
            qtd = int(r[1] or 0)
            custo_total = float(r[2] or 0)
            custo_unit = custo_total / qtd if qtd > 0 else 0
            nf_link = f"<a href='/comprovantes/{r[4]}' target='_blank' style='color:#0ea5e9; font-weight:bold; text-decoration:none;'>Ver NF</a>" if r[4] else "<span style='color:#555;'>-</span>"
            linhas += f"<tr><td style='color:#FFF; font-weight:bold;'>{r[0]}</td><td style='color:{COR_AMARELO}; text-align:center;'>{qtd}</td><td style='color:#FFF; text-align:right;'>R$ {custo_total:.2f}<br><small style='color:#888;'>R$ {custo_unit:.2f}/un</small></td><td style='color:#888; text-align:center;'>{r[3]}</td><td style='text-align:center;'>{nf_link}</td></tr>"
...
async def dashboard(request: Request, inicio: str = "", fim: str = "", tipo_venda: str = "TODOS"):
    ...
    with engine.connect() as conn:
        kpi = conn.execute(text(f"SELECT SUM(total_conta) FROM comandas WHERE {where_clause}"), params).fetchone()
        faturamento_filtrado = float(kpi[0]) if kpi and kpi[0] is not None else 0.0
        
        pag_q = conn.execute(text(f"SELECT forma_pagamento, SUM(total_conta) FROM comandas WHERE {where_clause} GROUP BY forma_pagamento"), params).fetchall()
        totais_pag = {"DINHEIRO": 0.0, "PIX": 0.0, "C. CREDITO": 0.0, "C. DEBITO": 0.0}
        for p in pag_q: 
            if p[0] in totais_pag: totais_pag[p[0]] = float(p[1] or 0)

        estornos_kpi = conn.execute(text(f"SELECT COUNT(*), SUM(valor) FROM vendas_itens WHERE {where_estorno}"), {"inicio": dt_inicio, "fim": dt_fim}).fetchone()
        qtd_estornada = int(estornos_kpi[0]) if estornos_kpi and estornos_kpi[0] is not None else 0
        total_estornado = float(estornos_kpi[1]) if estornos_kpi and estornos_kpi[1] is not None else 0.0
            
        itens_db = conn.execute(text(f"SELECT item_nome, COUNT(*), SUM(valor) FROM vendas_itens WHERE status = 'FECHADA' AND comanda_num IN (SELECT numero_comanda FROM comandas WHERE {where_clause}) GROUP BY item_nome ORDER BY COUNT(*) DESC"), params).fetchall()
        linhas_tabela = "".join([f"<tr><td style='color:#FFF; font-weight:bold;'>{it[0]}</td><td style='color:{COR_AMARELO}; text-align:center;'>{it[1]}</td><td style='color:#FFF; text-align:right;'>R$ {float(it[2] or 0):.2f}</td></tr>" for it in itens_db])
...
async def tela_usuarios(request: Request):
    ...
    linhas = ""
    with engine.connect() as conn:
        users_db = conn.execute(text("SELECT id, username, email, role, status FROM usuarios ORDER BY status DESC, username")).fetchall()
        for r in users_db:
            acoes = ""
            if r[1] != "admin":
                if r[4] == 'BLOQUEADO': btn_block = f"<form action='/toggle_usuario' method='post' style='margin:0;'><input type='hidden' name='id' value='{r[0]}'><button class='btn-acao' style='background:#10b981; padding:8px; margin:0;' title='Aprovar Acesso'>🔓</button></form>"
                else: btn_block = f"<form action='/toggle_usuario' method='post' style='margin:0;'><input type='hidden' name='id' value='{r[0]}'><button class='btn-acao' style='background:#f59e0b; padding:8px; margin:0;' title='Bloquear'>🔒</button></form>"
                
                btn_edit_acesso = f"<button type='button' class='btn-acao btn-dark' style='padding:8px; margin:0;' onclick='abrirModalAcesso({r[0]}, \"{r[1]}\", \"{r[3]}\")' title='Alterar Cargo do Membro'>⚙️</button>"
                btn_del = f"<form action='/excluir_usuario' method='post' style='margin:0;' onsubmit='return confirm(\"Excluir membro permanentemente?\");'><input type='hidden' name='id' value='{r[0]}'><button class='btn-acao btn-red' style='padding:8px; margin:0;'>🗑️</button></form>"
                acoes = f"<div style='display:flex; gap:5px;'>{btn_block}{btn_edit_acesso}{btn_del}</div>"
            st_badge = f"<span style='color:#10b981; font-weight:bold;'>ATIVO</span>" if r[4] == 'ATIVO' else f"<span style='color:{COR_VERMELHO}; font-weight:bold;'>PENDENTE/BLOQ.</span>"
            role_display = (r[3] or "MEMBRO").replace(',', ', ').upper()
            linhas += f"<tr><td><b style='color:#FFF;'>{r[1].upper()}</b><br><small style='color:#888;'>{r[2] or 'S/ Email'}</small></td><td style='color:{COR_AMARELO}; font-weight:bold; font-size:11px;'>{role_display}</td><td>{st_badge}</td><td>{acoes}</td></tr>"
    ...
    try:
        with engine.begin() as conn:
            ...
            if target and target[0] != 'admin':
                novo_status = 'ATIVO' if target[1] == 'BLOQUEADO' else 'BLOQUEADO'
